...
def worker(pid, user_id, num_months_to_learn):
    ...
    np.save('%08d_%02d_wall' % (user_id, months), wall_intensity_data)
    np.save('%08d_%02d_conn' % (user_id, months), conn_probability_data)
    ...
    np.save('%08d_%d_best' % (user_id, months), np.array(best_intensity))
...
if __name__ == '__main__':
    ...
    jobs = []
    for i in range(len(good_users[:1000])):
