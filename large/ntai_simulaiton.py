...
#     real_process = [(x - test_start_date_unix) / 3600. for x in real_process]
    learn_start_date = test_start_date - timedelta(days=month * 30)
    learn_end_date = test_start_date - timedelta(seconds=1)
    user_learned_intensity = user.tweet_list().sublist(learn_start_date, learn_end_date).get_periodic_intensity(24, learn_start_date, learn_end_date)
    
    real_process = generate_piecewise_constant_poisson_process(user_learned_intensity * total_days)
...
#         target_wall_no_offset = ((test_list._get_tweet_list() - test_start_date_unix) / 3600.).tolist()
        target_learned_wall_intensity = target.wall_tweet_list().sublist(learn_start_date, learn_end_date).get_periodic_intensity(24 , learn_start_date, learn_end_date)
    
        target_wall_no_offset = generate_piecewise_constant_poisson_process(target_learned_wall_intensity * total_days)
        
        target_learned_intensity = target.tweet_list().sublist(learn_start_date, learn_end_date).get_periodic_intensity(24 , learn_start_date, learn_end_date)
                                                                               
        target_tweets = TweetList(generate_piecewise_constant_poisson_process(target_learned_intensity * total_days))
        
        tweet_bags = target_tweets.get_periodic_intensity(n, test_start_date, test_end_date)
    ...
    if sum(before) == 0.:
        for month in [3]:
            np.save('%sbad_%08d_%02d_visibility_avm_simul' % (out_path_prefix, user.user_id(), month), [-1.])
        return
    ...
    for month in [3]:
        ...
        np.save('%s%08d_%02d_visibility_avm_simul' % (out_path_prefix, user.user_id(), month), [res])
